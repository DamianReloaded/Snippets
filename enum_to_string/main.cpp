...
constexpr EnumString<ObjectId, 8> ObjectIdConvert(ObjectIdMap); // Correct placement

int main()
{
    size_t b = 0;

    // Iterate through the enum and print values
    ObjectIdConvert.ForEach([&b](ObjectId id, std::string_view name) {
        b += ObjectIdConvert.ToSize(id); // Sum the enum values as integer values
        std::cout << name << " ,"; // Print the enum name
    });

    // Output the final accumulated sum of enum values
    std::cout << "\nSum of enum values: " << b << std::endl;
